...
class Video:
    def __init__(self, input_raw) -> None:
        ...
        if not self.cap.isOpened():
            raise VException("Invalid video input path or file format specified")

        self.props = get_cap_properties(self.cap)

        if self.props["frames"] < 0 or self.props["fps"] > 1000:
            raise VException(f"Input does not contain a video source!")
